 ...
 * - React Query para operações online
 * - localStorage para cache de leitura offline (simples e sem conflitos!)
 * - Operações (criar/editar/deletar) APENAS online
 */
export function useTasks() {
  ...
  const createMutation = useMutation({
    mutationFn: async (newTask: Omit<Task, 'id' | 'created_at' | 'updated_at'>) => {
      if (!navigator.onLine) {
        throw new Error('Sem conexão - não é possível criar tarefas offline');
      }
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, updates }: { id: number; updates: Partial<Task> }) => {
      if (!navigator.onLine) {
        throw new Error('Sem conexão - não é possível atualizar tarefas offline');
      }
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: number) => {
      if (!navigator.onLine) {
        throw new Error('Sem conexão - não é possível deletar tarefas offline');
      }
